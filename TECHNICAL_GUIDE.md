# AI-SOC Platform — Tài liệu kỹ thuật toàn diện
...
**AI-SOC** là nền tảng Security Operations Center (SOC) tích hợp AI, phục vụ:
...
- **Tự động phản ứng** — Auto-block IP nguy hiểm qua iptables/SSH remote với target cấu hình được (local, Suricata, Agent)
...
┌───────────────────────▼──────────────────────────────────────────┐
│  VPS Dashboard + AI + Wazuh Manager (103.98.152.207)             │
│  ├─ Nginx (:80)                                                  │
│  │   ├─ /         → static/   (HTML+CSS+JS)                     │
...
│  └─ iptables INPUT chain (có thể tắt qua block target flags)     │
└───────────────────────┬──────────────────────────────────────────┘
                        │ (SSH remote block qua port 2222)
┌───────────────────────▼──────────────────────────────────────────┐
│  VPS Wazuh Agent (103.9.159.120)                                 │
│  ├─ Wazuh Agent                                                  │
│  ├─ SSH :2222                                                    │
│  └─ iptables AI_BLOCK chain (target block chính cho demo)        │
└──────────────────────────────────────────────────────────────────┘
```
...
│   │   ├── settings.py           # Lưu cấu hình feeds + general settings
...
│   │   ├── settings_store.py     # JSON store cho settings UI
...
│       └── platform_settings.json # Feed settings / general settings
│
└── static/                       # Frontend SPA
    ...
    └── js/
        ...
        ├── blocked_ips_page.js   # Trang quản lý IP bị chặn
...
### VPS Wazuh Agent — ubuntu (103.9.159.120)

| Thành phần | Chi tiết |
|------------|----------|
| Wazuh Agent | Gửi auth/system logs về Wazuh Manager |
| SSH | Port `2222` |
| iptables | Chain `AI_BLOCK` để nhận remote block |
| Vai trò demo | Target block chính khi `BLOCK_TARGET_AGENT=true` |
...
| `/api/auth/change-password` | POST | Đổi mật khẩu user hiện tại + force re-login |
| `/api/auth/users` | GET/POST/PATCH/DELETE | CRUD users (admin only) |
| `/api/auth/roles` | GET | Danh sách roles & quyền |

#### `routers/settings.py` — Runtime Settings

| Endpoint | Method | Mô tả |
|----------|--------|-------|
| `/api/settings/feeds` | GET | Danh sách cấu hình feed (API key được mask) |
| `/api/settings/feeds/{feed_id}` | GET/PUT | Đọc / cập nhật feed config |
| `/api/settings/general` | GET/PUT | Cấu hình chung (whitelist, SSH port...) |
...
| `/api/threatintel/sync/{feed_id}` | POST | Xóa cache / sync feed theo yêu cầu |
...
**Bổ sung mới:** `get_recent_alerts()` enrich lại `data.src_ip` từ `data.srcip`, `full_log`, `location` cho các auth logs kiểu SSH/PAM. Điều này giúp UI hiển thị đúng IP nguồn dù Wazuh không map sẵn `data.src_ip`.
...
2. Check whitelist (private IPs + VPS IPs + admin IP từ config)
3. Check cache (đã block trước đó?)
4. Kiểm tra block targets (`local`, `suricata`, `agent`)
5. Chỉ gọi iptables/SSH trên target đang bật
...
Ngoài whitelist cứng, hệ thống còn merge thêm `ADMIN_WHITELIST_IPS`, `suricata_vps_host`, `agent_vps_host` từ config/runtime settings.
...
- Đổi mật khẩu ngay trên trang Settings → gọi `/api/auth/change-password` và buộc đăng nhập lại sau khi đổi thành công
...
- **Feeds:** 4 feed cards (AbuseIPDB, Wazuh + Suricata, AI Engine, VirusTotal)

**Bổ sung mới:**
- Feed cards có nút `Cấu hình` mở modal runtime settings
- Settings lưu vào `backend/data/platform_settings.json`
- API key được mask khi trả về UI

#### `blocked_ips_page.js` — Blocked IP Management

- Trang riêng để xem, lọc, unblock, lookup IP đã chặn
- Badge số lượng IP bị chặn trên sidebar
- Dữ liệu đọc từ `/api/blocked-ips`
...
```
Wazuh Agents / Suricata Sensors / Auth logs
    ↓ (logs)
...
pipeline.py:
    ...
    └─ should_block=True → nếu ai_block_auto=True → firewall.block_ip()
```
...
routers/api.py → firewall.block_ip()
    ├─ 1. Validate IP
    ├─ 2. Check whitelist
    ├─ 3. Xác định target nào đang bật (local / suricata / agent)
    ├─ 4. Chỉ thực thi trên target được bật
    ├─ 5. SSH → iptables trên VPS Agent hoặc Suricata (nếu bật)
    └─ 6. Log + cache + response
```
...
`backend/data/platform_settings.json` lưu các cấu hình thay đổi từ UI:
- Threat intel feeds (API key, enabled, sync interval, cache TTL)
- General settings (admin whitelist IPs, SSH protected port)
...
2. Backend: SHA-256 verify → tạo session token random → lưu sessions table
...
6. Change password: xóa toàn bộ session của user → bắt đăng nhập lại
```

**Default admin:** `admin / admin123` — **đổi ngay sau khi deploy** qua UI Settings hoặc `/api/auth/change-password`.
...
# Sau khi vào hệ thống nên đổi mật khẩu ngay trong Settings
```
